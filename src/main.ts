...
class HeishamonAdapter extends AdapterBase {
    ...
    if (!cfg.readOnlyMode) {
      ...
      this.poller = new Poller({
        ...
        timers: {
          ...
          clearTimeout: (handle) => this.clearTimeout(handle as ioBroker.Timeout),
        },
      });
      this.poller.start();
    } else {
  ...
  private clampOptionalMs(
    ...
    if (value < min || value > MAX_TIMER_MS) {
      ...
      this.log.warn(
        `config.${label}=${value} out of range [${min}, ${MAX_TIMER_MS}], clamping to ${clamped}`,
      );
      return clamped;
    }
    return value;
   ...
   */
  private clampRetries(value: number | undefined): number | undefined | null {
    if (value === undefined) {
      return undefined;
    }
    ...
    if (value > MAX_SEND_RETRIES) {
      this.log.warn(
        `config.sendMaxRetries=${value} exceeds ${MAX_SEND_RETRIES}, clamping to ${MAX_SEND_RETRIES}`,
      );
