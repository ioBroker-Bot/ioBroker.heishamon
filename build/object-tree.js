 ...
 */
import { ALL_DATAPOINTS, } from './protocol/index.js';
...
function determineRole(datapoint, commonType) {
    ...
    // signed-byte decoder.
    const looksLikeTemperature = name.endsWith('_Temp') || (decoder === 'getIntMinus128' && unit === '°C');
 ...
 */
export function stateId(frameSource, name) {
    return `${frameSource}.${name}`;
}
function buildStateDefinition(datapoint) {
