 ...
 */
const INITIAL_HANDSHAKE_PAYLOAD = [0x31, 0x05, 0x10, 0x01, 0x00, 0x00, 0x00];
/**
 ...
 */
export function identifyFrame(frame) {
    if (frame.length < 4) {
        return null;
    }
    for (const type of Object.keys(FRAME_HEADERS)) {
...
export function createTemplate(type) {
    switch (type) {
        ...
        case 'mainResponse':
        case 'extraResponse':
            throw new RangeError(`cannot create a send-side template for ${type}: response frames are emitted by the heat pump`);
    }
}
/**
 ...
 */
export function buildFrame(type, mutate) {
 ...
 */
function buildZeroPaddedTemplate(type) {
