 ...
 */
export function encodeSetCommand(name, value) {
...
function applyRule(payload, name, rule, value) {
    switch (rule.kind) {
        case 'tempOffset':
...
function encodeTempOffset(name, rule, value) {
...
function encodeEnumByte(name, rule, value) {
    requireFiniteInteger(name, value);
    if (value < 0 || value >= rule.mapping.length) {
        throw new Error(`value ${value} out of range for "${name}" (allowed: 0..${rule.mapping.length - 1})`);
    }
    return rule.mapping[value];
}
function encodeLinear(name, rule, value) {
    requireFiniteInteger(name, value);
    if (value < rule.min || value > rule.max) {
        throw new Error(`value ${value} out of range for "${name}" (allowed: ${rule.min}..${rule.max})`);
    }
    return (value + rule.offset) & 0xff;
}
function requireFiniteInteger(name, value) {
